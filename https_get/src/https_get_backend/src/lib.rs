//post code:https://github.com/dfinity/examples/blob/master/rust/send_http_post/src/send_http_post_backend/src/lib.rs
...
async fn get_icp_usd_exchange() -> String {
    ...
    let url = format!(
        ...
        (start_timestamp + seconds_of_time).to_string(),
        seconds_of_time.to_string()
    );

    ic_cdk::api::print(format!("Request URL: {}", url));
    ...
    let transform_context = TransformContext::from_name("transform".to_string(), serde_json::to_vec(&context).unwrap());
    ...
    let request = CanisterHttpRequestArgument {
        ...
        transform: Some(transform_context),//optional for request
    ...
    let cycles: u128 = 20_949_972_000;
    match http_request(request, cycles).await {
        ...
        Ok((response,)) => {
            ...
            ic_cdk::api::print(format!("Response Body: {}", str_body));
        ...
        Err((r, m)) => {
            let message = format!(
                "The http_request resulted in an error. RejectionCode: {r:?}, Error: {m}"
            );
